...
class FastDownward_LAMA (Planner):
	...
	def execute( self ) :
		if LAMA.greedy :
			...
			#cmd_string = './plan %s %s %s'%( self.domain, self.problem, self.result)
			cmd_string = '/home/mk/Planning/fastdownwardplanner/fast-downward.py  --alias lama-first %s %s '%( self.domain, self.problem)  #Michael. I could put the normal lama here?
		self.log = benchmark.Log( self.log_file )
		self.signal, self.time = benchmark.run( cmd_string, self.max_time, self.max_mem, self.log )
		self.gather_data()
	...
	def gather_data( self ) :
		...
		for line in instream :
			...
			if "Search time" in line :
				tokens = line.split( " " )
				tokens[2] = tokens[2].replace("s", "") #Michael FD planner has output timeformat "1.346s", where the "s" causes trouble
				self.search_time += float( tokens[2] )
			if "Search iteration" in line :
				self.iteration += 1
